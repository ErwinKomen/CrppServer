  ...
  public DataObject handle() {
    try {
      ...
      if (!jReq.has("userid")) return DataObject.errorObject("syntax", 
          "The /dbget request must contain: userid.");
      sCurrentUserId = jReq.getString("userid");
      // Check for the name of the database
      String sDbName = "";
      if (!jReq.has("name")) return DataObject.errorObject("syntax", 
          "The /dbget request must contain: name.");
      ...
      if (arDbList.isEmpty()) {
        return DataObject.errorObject("not_found", 
          "The .xml file requested is not available.");
      } 
      // Locate the file
      ...
      objContent.put("db", (new FileUtil()).readFile(fDbPath));
